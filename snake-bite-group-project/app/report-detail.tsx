...
import { SNAKES_DATABASE, UNKNOWN_SNAKE_ID, isUnknownSnake, getSnakeById } from '../types/snake';
...
export default function ReportDetailScreen() {
  ...
  const loadReport = async () => {
    try {
      ...
      console.log('Loaded report:', loadedReport);

      if (loadedReport) {
        console.log('Snake ID:', loadedReport.snakeId);
        console.log('Is unknown snake:', loadedReport.snakeId === UNKNOWN_SNAKE_ID || loadedReport.snakeId === 0);
      }
  ...
  // Check if this is a known snake in our database
  const isKnownSnake = report?.snakeId !== undefined &&
    report.snakeId !== null &&
    report.snakeId !== UNKNOWN_SNAKE_ID &&
    report.snakeId !== 0;

  // Get the snake from database if known
  const databaseSnake = isKnownSnake ? getSnakeById(report!.snakeId!) : null;

  const handleViewSnakeGuide = () => {
    if (isKnownSnake && databaseSnake) {
      ...
      Alert.alert(
        ...
        `"${report?.scientific || 'Unknown'}" was not found in our local database. Would you like to search our snake guide?`,
        [
          { text: 'Cancel', style: 'cancel' },
          {
            text: 'Search Guide',
  ...
  // Determine which image to show as reference
  const getReferenceImageUri = (): string => {
    // For known snakes, use the database image
    if (isKnownSnake && databaseSnake) {
      return databaseSnake.imageUrl;
    }
    // For unknown snakes, use the user's submitted photo if available
    if (report?.snakePhotoUri) {
      return report.snakePhotoUri;
    }
    // Fall back to the stored snakeImageUrl (which might be a placeholder)
    if (report?.snakeImageUrl) {
      return report.snakeImageUrl;
    }
    // Final fallback
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <ScrollView style={styles.content}>
            ...
            <View style={{ flex: 1 }}>
              ...
              {!isKnownSnake && (
                <View style={styles.unknownBadge}>
                  ...
                  <Text style={styles.unknownBadgeText}>Not in database</Text>
                </View>
              )}
            </View>
          </View>

          <View style={[styles.dangerBadge, { backgroundColor: report.venomous ? '#dc2626' : (isKnownSnake ? '#16a34a' : '#64748b') }]}>
            <Ionicons name={report.venomous ? "warning" : (isKnownSnake ? "shield-checkmark" : "help")} size={16} color="#ffffff" />
            <Text style={styles.dangerText}>
              {report.venomous ? 'VENOMOUS' : (isKnownSnake ? 'NON-VENOMOUS' : 'UNKNOWN')} - {report.dangerLevel}
            </Text>
          </View>

          {report.confidence && (
            <View style={[styles.confidenceBadge, { backgroundColor: isKnownSnake ? '#16a34a' : '#f59e0b' }]}>
              <Text style={styles.confidenceText}>
                {report.confidence}% Match Confidence {!isKnownSnake && '(Unverified)'}
              </Text>
            </View>
          )}
        </View>
          ...
          <View style={styles.analysisGrid}>
            ...
            <View style={styles.analysisItem}>
              ...
              <Text style={[styles.analysisValue, { color: report.venomous ? '#dc2626' : '#16a34a' }]}>
                {report.venomous ? 'YES' : (isKnownSnake ? 'NO' : 'UNKNOWN')}
              </Text>
            </View>

            <View style={styles.analysisItem}>
              <Text style={[styles.analysisLabel, { color: colors.textSecondary }]}>In Database</Text>
              <Text style={[styles.analysisValue, { color: isKnownSnake ? '#16a34a' : '#f59e0b' }]}>
                {isKnownSnake ? 'YES' : 'NO'}
              </Text>
            </View>
          </View>

          {/* Snake Guide Button - Different behavior for known vs unknown */}
          <TouchableOpacity
            style={[styles.guideButton, { backgroundColor: isKnownSnake ? '#2563eb' : '#64748b' }]}
            onPress={handleViewSnakeGuide}
          >
            <Ionicons name={isKnownSnake ? "book" : "search"} size={20} color="#ffffff" />
            <Text style={styles.guideButtonText}>
              {isKnownSnake ? 'View Species Guide' : 'Search Snake Guide'}
            </Text>
            <Ionicons name="arrow-forward" size={20} color="#ffffff" />
          </TouchableOpacity>
        </View>

        {/* Reference Image - Only show for known snakes */}
        {isKnownSnake && databaseSnake && (
          <View style={[styles.section, { backgroundColor: colors.cardBg, borderColor: colors.border }]}>
            ...
            <TouchableOpacity onPress={() => openFullscreenImage(databaseSnake.imageUrl)}>
              <Image
                source={{ uri: databaseSnake.imageUrl }}
        ...
        {/* Warning for unknown snakes */}
        {!isKnownSnake && (
          <View style={[styles.warningBox, { backgroundColor: '#fef3c7', borderColor: '#f59e0b' }]}>
            <Ionicons name="information-circle" size={24} color="#f59e0b" />
            <View style={{ flex: 1 }}>
              <Text style={[styles.warningTitle, { color: '#92400e' }]}>Unverified Species</Text>
              <Text style={[styles.warningText, { color: '#a16207' }]}>
                This snake was not found in our database. The identification is based on AI analysis only.
                Please consult an expert for verification and treat this snake as potentially dangerous.
              </Text>
            </View>
          </View>
        )}

        {/* Warning for venomous snakes */}
        {report.venomous && (
